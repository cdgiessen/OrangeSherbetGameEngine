...
void Light::FillUniform(GLSLProgram *shader, int i, glm::mat4 eyeSpace) {
	...
	switch (this->lightType) {
		...
		case(LightType::Dir):
			name = "dirlights[" + std::to_string(i) + "].direction";
			shader->setUniform(name.c_str(), glm::vec3(eyeSpace*glm::vec4(direction, 1.0f)));
		
			name = "dirlights[" + std::to_string(i) + "].color";
			shader->setUniform(name.c_str(), GetColor().ToVec3());
		
			name = "dirlights[" + std::to_string(i) + "].intensity";
			shader->setUniform(name.c_str(), intensity);
			break;
		
		case(LightType::Spot):
			name = "spotLights[" + std::to_string(i) + "].position";
			shader->setUniform(name.c_str(), eyeSpace*glm::vec4(position, 1.0f));

			name = "spotLights[" + std::to_string(i) + "].direction";
			shader->setUniform(name.c_str(), glm::vec3(eyeSpace*glm::vec4(direction, 1.0f)));

			name = "spotLights[" + std::to_string(i) + "].color";
			shader->setUniform(name.c_str(), GetColor().ToVec3());

			name = "spotLights[" + std::to_string(i) + "].intensity";
			shader->setUniform(name.c_str(), intensity);

			name = "spotLights[" + std::to_string(i) + "].cutOff";
			shader->setUniform(name.c_str(), cutOff);

			name = "spotLights[" + std::to_string(i) + "].outerCutOff";
